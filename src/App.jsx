      ...
      <div style={{ padding:'16px 16px 120px' }}>
        ...
        {view==='home'&&<>
          {!ricariche.length?(
            ...
            // Calcola progressivo CASA globale per ogni mese (ordinato cronologicamente)
            ...
            const progMese = acc;
            const progCasa = progCasaPerMese[key];
                ...
                <div onClick={toggleMese} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 16px', background:'linear-gradient(135deg,rgba(0,229,255,0.1),rgba(124,58,237,0.1))', border:`1px solid ${S.border}`, borderRadius: aperto ? '14px 14px 0 0' : 14, cursor:'pointer', userSelect:'none' }}>
                  ...
                  <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                    <div style={{ textAlign:'right' }}>
                      ...
                      <div style={{ fontFamily:'monospace', fontSize:'0.68rem', color:'#f59e0b' }}>📅 €{(progMese||0).toFixed(2)}</div>
                      <div style={{ fontFamily:'monospace', fontSize:'0.68rem', color:'#34d399' }}>🏠 €{(progCasa||0).toFixed(2)}</div>
                    </div>
